...
export default function PhotographyExcursionsPage() {
  ...
  const [selectedExcursion, setSelectedExcursion] = useState<PhotographyExcursion | null>(null);
  ...
  const handleBookingSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await conciergeService.submitConciergeEnquiry({
        ...
        experience_type: "Photography Excursion",
      ...
      <main>
                ...
                {filteredExcursions.map((excursion) => (
                  <div key={excursion.id} onClick={() => setSelectedExcursion(excursion)} className="bg-white rounded-2xl border border-background-200/70 hover:border-primary-200/60 overflow-hidden group cursor-pointer transition-all">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-background-200/70">
                      <div>
                        <span className="text-lg font-bold text-foreground-900">€{excursion.pricePerPerson}</span>
        ...
        {selectedExcursion && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-foreground-950/60 backdrop-blur-sm" onClick={() => setSelectedExcursion(null)}></div>
            <div className="relative bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto z-10">
              <button onClick={() => setSelectedExcursion(null)} className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/90 backdrop-blur-sm border border-background-200 text-foreground-600 hover:text-foreground-900 transition-all z-20 cursor-pointer">
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accent-100 text-accent-700 text-xs font-medium mb-2">
                      <i className="ri-camera-lens-line text-[11px]"></i>{selectedExcursion.focus}
                    </span>
                    <h2 className="font-heading text-2xl text-foreground-900">{selectedExcursion.name}</h2>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedExcursion.locations.map((loc) => (
                      <span key={loc} className="px-3 py-1.5 rounded-full bg-secondary-100 text-secondary-800 text-xs font-medium whitespace-nowrap flex items-center gap-1">
                        <i className="ri-map-pin-line text-[10px]"></i>{loc}
                      </span>
                    ))}
                  </div>
                </div>
        ...
        <RelatedExperiences currentPage="photography-excursions" />
      </main>
      <Footer />
    </>
  );
