...
export default function PersonalDriverPage() {
  ...
  const [selectedDriver, setSelectedDriver] = useState<PersonalDriver | null>(null);
  ...
  const filteredDrivers = useMemo(() => {
    let results = personalDrivers;
    if (activeType !== "all") {
      const typeMap: Record<string, string> = {
        "luxury-sedan": "Luxury Sedan",
        "suv-van": "SUV & Van",
        "airport-transfer": "Airport Transfer",
        "full-day": "Full Day Hire",
      };
  ...
  const handleBookingSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const driverName = (formData.get("driver_name") as string || selectedDriver?.name || "");
      ...
      const result = await conciergeService.submitConciergeEnquiry({
        ...
        experience_type: "Personal Driver",
      ...
      <main>
        ...
        <section className="w-full px-4 md:px-8 lg:px-12 py-4 bg-background-50">
          <div className="max-w-7xl mx-auto">
            {!isLoading && !fetchError && (
              <div><p className="text-sm text-foreground-500">{filteredDrivers.length} {filteredDrivers.length === 1 ? "driver listing" : "driver listings"}</p><OfferProvenanceNotice /></div>
            )}
          </div>
        </section>
        ...
        {selectedDriver && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-foreground-950/60 backdrop-blur-sm" onClick={() => setSelectedDriver(null)}></div>
            <div className="relative bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto z-10">
              <button onClick={() => setSelectedDriver(null)} className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/90 backdrop-blur-sm border border-background-200 text-foreground-600 hover:text-foreground-900 transition-all z-20 cursor-pointer">
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accent-100 text-accent-700 text-xs font-medium mb-2">
                      <i className="ri-car-line text-[11px]"></i>{selectedDriver.vehicleType}
                    </span>
        ...
        <RelatedExperiences currentPage="personal-driver" />
      </main>
      <Footer />
    </>
  );
