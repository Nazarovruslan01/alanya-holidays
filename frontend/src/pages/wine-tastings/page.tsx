...
export default function WineTastingsPage() {
  ...
  const filteredTastings = useMemo(() => {
    let results = wineTastings;
    if (activeStyle !== "all") {
      if (activeStyle === "anatolian") results = results.filter((t) => t.wineTypes.every((w) => ["Öküzgözü", "Boğazkere", "Kalecik Karası", "Narince", "Emir", "Bornova Misketi", "Çalkarası"].includes(w)) || t.wineTypes.some((w) => ["Öküzgözü", "Boğazkere", "Kalecik Karası", "Narince", "Emir", "Bornova Misketi", "Çalkarası"].includes(w)));
      else if (activeStyle === "blend") results = results.filter((t) => t.wineTypes.some((w) => !["Öküzgözü", "Boğazkere", "Kalecik Karası", "Narince", "Emir", "Bornova Misketi", "Çalkarası"].includes(w) || t.name.includes("Old World")));
      else if (activeStyle === "food") results = results.filter((t) => t.foodPairing.length > 40);
      else if (activeStyle === "private") results = results.filter((t) => t.groupSize.startsWith("2") && t.pricePerPerson >= 110);
    }
    if (sortBy === "rating") results = [...results].sort((a, b) => b.rating - a.rating);
      ...
      const aH = parseFloat(a.duration); const bH = parseFloat(b.duration);
      return aH - bH;
    });
    return results;
  ...
  const handleBookingSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const tastingName = (formData.get("tasting_name") as string || selectedTasting?.name);
      ...
      <main>
        ...
        {selectedTasting && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-foreground-950/60 backdrop-blur-sm" onClick={() => setSelectedTasting(null)}></div>
            <div className="relative bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto z-10">
              <button onClick={() => setSelectedTasting(null)} className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/90 backdrop-blur-sm border border-background-200 text-foreground-600 hover:text-foreground-900 transition-all z-20 cursor-pointer">
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accent-100 text-accent-700 text-xs font-medium mb-2">
                      <i className="ri-goblet-line text-[11px]"></i>{selectedTasting.winesIncluded}
                    </span>
                    <h2 className="font-heading text-2xl text-foreground-900">{selectedTasting.name}</h2>
                  </div>
        ...
        <RelatedExperiences currentPage="wine-tastings" />
      </main>
      <Footer />
    </>
  );
