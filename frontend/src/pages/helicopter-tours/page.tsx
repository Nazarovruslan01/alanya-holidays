...
export default function HelicopterToursPage() {
  ...
  const [selectedTour, setSelectedTour] = useState<HelicopterTour | null>(null);
  ...
  const filteredTours = useMemo(() => {
    ...
    if (activeDuration !== "all") {
      if (activeDuration === "20-30") results = results.filter((t) => parseInt(t.duration) >= 20 && parseInt(t.duration) <= 30);
      else if (activeDuration === "45-60") results = results.filter((t) => parseInt(t.duration) >= 45 && parseInt(t.duration) <= 60);
      else if (activeDuration === "90-plus") results = results.filter((t) => parseInt(t.duration) >= 90);
    }
      ...
      <main>
        ...
        <section className="w-full px-4 md:px-8 lg:px-12 pt-8 pb-4 bg-background-50">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide flex-wrap">
              {tourDurations.map((d) => (
                <button key={d.id} onClick={() => setActiveDuration(d.id)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap cursor-pointer ${activeDuration === d.id ? "bg-primary-500 text-white" : "bg-white border border-foreground-200 text-foreground-600 hover:border-primary-200 hover:text-foreground-900"}`}>
                  <i className={`${d.icon} text-sm`}></i>{d.name}
                </button>
              ))}
        ...
        {selectedTour && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-foreground-950/60 backdrop-blur-sm" onClick={() => setSelectedTour(null)}></div>
            <div className="relative bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto z-10">
              <button onClick={() => setSelectedTour(null)} className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/90 backdrop-blur-sm border border-background-200 text-foreground-600 hover:text-foreground-900 transition-all z-20 cursor-pointer">
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accent-100 text-accent-700 text-xs font-medium mb-2">
                      <i className="ri-time-line text-[11px]"></i>{selectedTour.duration}
                    </span>
                    <h2 className="font-heading text-2xl text-foreground-900">{selectedTour.name}</h2>
                  </div>
        ...
        <RelatedExperiences currentPage="helicopter-tours" />
      </main>
      <Footer />
    </>
  );
