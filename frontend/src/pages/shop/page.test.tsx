...
describe("ShopPage cart identity", () => {
  ...
  it("adds the canonical catalog product ID and image to the cart", async () => {
    ...
    fireEvent.click(
      within(screen.getByRole("link", { name: /Ceramic Vase/ })).getByRole(
        "button",
        { name: /add to cart/i },
      ),
    );
    ...
    expect(shopMocks.showToast).toHaveBeenCalledWith(
      "Add to Cart",
      "Ceramic Vase",
      "success",
    );
  ...
  it("disables sold-out products and guards their add-to-cart handler", async () => {
    render(
      <MemoryRouter>
        <ShopPage />
      </MemoryRouter>,
    ...
    await screen.findByRole("heading", { name: "Sold Out Bowl" });
    const soldOutButton = screen.getByRole("button", { name: "Out of Stock" });

    expect(soldOutButton).toBeDisabled();
    fireEvent.click(soldOutButton);

    expect(shopMocks.addToCart).not.toHaveBeenCalled();
    expect(shopMocks.showToast).not.toHaveBeenCalled();
  });
});
