...
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('event-media-staging', 'event-media-staging', false, 52428800, ARRAY['video/mp4', 'video/webm'])
ON CONFLICT (id) DO UPDATE
SET name = EXCLUDED.name,
    public = EXCLUDED.public,
    file_size_limit = EXCLUDED.file_size_limit,
...
DROP POLICY IF EXISTS "event_media_staging_select" ON storage.objects;
DROP POLICY IF EXISTS "event_media_staging_insert" ON storage.objects;
DROP POLICY IF EXISTS "event_media_staging_update" ON storage.objects;
DROP POLICY IF EXISTS "event_media_staging_delete" ON storage.objects;
...
CREATE TABLE IF NOT EXISTS public.event_media (
  ...
  bucket TEXT NOT NULL CHECK (bucket IN ('forum-media', 'event-media-staging', 'event-media')),
  ...
  state TEXT NOT NULL DEFAULT 'pending'
    CHECK (state IN ('pending', 'promoting', 'ready', 'attached', 'cleanup_pending', 'failed')),
  expires_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CHECK (
    (kind = 'image' AND bucket = 'forum-media' AND mime_type = 'image/webp' AND thumbnail_path IS NOT NULL AND thumbnail_url IS NOT NULL AND content_sha256 IS NULL)
    OR
    (
      kind = 'video'
      ...
      AND (
        (
          ...
          AND state IN ('pending', 'promoting', 'cleanup_pending', 'failed')
          AND (state <> 'promoting' OR content_sha256 IS NOT NULL)
        )
        OR
        (bucket = 'event-media' AND state IN ('ready', 'attached', 'cleanup_pending', 'failed'))
      )
    )
  )
);
...
CREATE INDEX IF NOT EXISTS idx_event_media_expiry ON public.event_media (expires_at, id) WHERE state IN ('pending', 'promoting', 'ready');
...
BEGIN
  ...
  WHERE owner_id = p_owner_id
    AND kind = 'video'
    AND state IN ('pending', 'promoting', 'ready', 'cleanup_pending');
  ...
  INSERT INTO public.event_media (
    ...
    p_id, p_owner_id, NULL, 'video', 'event-media-staging', p_object_path, NULL,
...
CREATE OR REPLACE FUNCTION public.complete_event_video_promotion(
  p_media_id UUID,
  p_owner_id UUID
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_media public.event_media%ROWTYPE;
  v_staging_not_before TIMESTAMPTZ;
BEGIN
  SELECT * INTO v_media
  FROM public.event_media
  ...
  IF v_media.kind = 'video'
    AND v_media.state = 'ready'
    AND v_media.bucket = 'event-media'
  THEN
    RETURN to_jsonb(v_media);
  END IF;
  IF v_media.kind <> 'video'
    OR v_media.state <> 'promoting'
    OR v_media.bucket <> 'event-media-staging'
    OR v_media.content_sha256 IS NULL
  THEN
    RETURN NULL;
  END IF;

  v_staging_not_before := GREATEST(
    now(),
    COALESCE(v_media.expires_at, now())
  );
  UPDATE public.event_media
  SET bucket = 'event-media',
      state = 'ready',
  ...
  -- The upload token targets staging and may still be live after promotion.
  -- Keep an independent tombstone until the original lease expires so a late
  -- token use cannot leave an untracked private object.
  INSERT INTO public.event_media_cleanup_outbox (
    media_id, bucket, object_paths, next_attempt_at
  )
  SELECT NULL, 'event-media-staging', ARRAY[v_media.object_path], v_staging_not_before
  WHERE NOT EXISTS (
    SELECT 1 FROM public.event_media_cleanup_outbox
    WHERE media_id IS NULL
      AND bucket = 'event-media-staging'
      AND object_paths = ARRAY[v_media.object_path]
...
END;
$$;
REVOKE ALL ON FUNCTION public.complete_event_video_promotion(UUID, UUID)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.complete_event_video_promotion(UUID, UUID)
  TO service_role;
...
BEGIN
  ...
  v_not_before := CASE
    WHEN v_media.kind = 'video'
      THEN GREATEST(
        ...
        COALESCE(v_media.expires_at, now()),
        CASE
          WHEN v_media.state = 'promoting' THEN now() + interval '24 hours'
          ELSE now()
        END
      )
    ELSE now()
  ...
  IF v_media.kind = 'video' THEN
    v_counterpart_bucket := CASE v_media.bucket
      WHEN 'event-media-staging' THEN 'event-media'
      ELSE 'event-media-staging'
    END;
    -- Copy and database promotion cannot be one Storage/SQL transaction. A
    -- detached counterpart tombstone closes both copy-before-DB and
    -- abandon-during-copy races without ever linking cleanup to ready media.
    INSERT INTO public.event_media_cleanup_outbox (
      media_id, bucket, object_paths, next_attempt_at
    )
    SELECT NULL, v_counterpart_bucket, ARRAY[v_media.object_path], v_not_before
    WHERE NOT EXISTS (
      SELECT 1 FROM public.event_media_cleanup_outbox
      WHERE media_id IS NULL
        AND bucket = v_counterpart_bucket
        AND object_paths = ARRAY[v_media.object_path]
        AND status <> 'completed'
    );
  END IF;
...
BEGIN
  ...
  IF NOT FOUND OR v_state NOT IN ('pending', 'promoting', 'ready', 'cleanup_pending', 'failed') THEN RETURN false; END IF;
...
BEGIN
  ...
  END IF;
  IF v_media.kind <> p_kind
    OR v_media.state <> 'ready'
    OR v_media.event_id IS NOT NULL
    OR (p_kind = 'image' AND v_media.bucket <> 'forum-media')
    OR (p_kind = 'video' AND v_media.bucket <> 'event-media')
    OR (p_kind = 'video' AND v_media.content_sha256 IS NULL)
  THEN
...
BEGIN
  FOR v_media IN SELECT id FROM public.event_media
    WHERE state IN ('pending', 'promoting', 'ready') AND expires_at <= now()
